...
// -----------------------------
// Get collection by name using v2 API
// -----------------------------
async function getCollectionByName(collectionName) {
  try {
    const response = await axios.get(`http://localhost:8000/api/v2/collections/${collectionName}`);
    return response.data;
  } catch (error) {
...
async function main() {
  try {
    ...
    if (headings.length > 0) {
      const headingChunks = splitText(headings.join(' '), 500);
      // Just store first 2 chunks for testing
      const chunksToStore = headingChunks.slice(0, 2);
      await storeEmbeddingsV2(collection.id, chunksToStore, 'HEADINGS');
    }
    
    if (bodyText && bodyText.length > 0) {
      const bodyChunks = splitText(bodyText, 1000);
      // Just store first chunk for testing
      const chunksToStore = bodyChunks.slice(0, 1);
      await storeEmbeddingsV2(collection.id, chunksToStore, 'BODY');
    }
