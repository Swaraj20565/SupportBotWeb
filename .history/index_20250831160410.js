...
// -----------------------------
// Initialize Chroma client (connect to Docker server)
// -----------------------------
const chroma = new ChromaClient({
  url: 'http://localhost:8000', // Chroma Docker server URL
});
...
// -----------------------------
// Scrape webpage: headings, body, links
// -----------------------------
async function scrapeWebpage(url = '') {
  try {
    const { data } = await axios.get(url);
...
async function generateAndStoreEmbeddings(textChunks, label = 'TEXT') {
  try {
    for (const [i, chunk] of textChunks.entries()) {
      ...
      console.log(`================ GEMINI EMBEDDING ${label} CHUNK ${i + 1} ================`);
      console.log(vector[0].slice(0, 10), '...'); // Preview first 10 values
      console.log('=================================================');
...
(async () => {
  ...
  console.log('✅ All embeddings stored in ChromaDB successfully.');
})();
