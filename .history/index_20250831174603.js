...
//     // Alternative model names to try if this fails:
//     // "gemini-1.0-pro"
//     // "gemini-pro"
...
// -----------------------------
// Generate vector embeddings for text (LOCAL)
// -----------------------------
async function generateVectorEmbeddings(text) {
  try {
    if (!embedder) {
      console.log('🔄 Loading local embedding model...');
      embedder = await pipeline('feature-extraction', 'Xenova/all-MiniLM-L12-v2'); // Changed to L12 for 768 dimensions
    }
    const output = await embedder(text, { pooling: 'mean', normalize: true });
    return Array.from(output.data);
  } catch (error) {
    console.error('❌ Failed to generate local embeddings:', error.message);
    throw error;
  }
}

// -----------------------------
// Generate embeddings for multiple documents (LOCAL)
// -----------------------------
async function generateEmbeddings(textChunks) {
  console.log(`🧠 Generating LOCAL embeddings for ${textChunks.length} chunks...`);
  try {
    if (!embedder) {
      console.log('🔄 Loading local embedding model...');
      embedder = await pipeline('feature-extraction', 'Xenova/all-MiniLM-L12-v2'); // Changed to L12 for 768 dimensions
...
// -----------------------------
// Generate answer using free API alternative
// -----------------------------
async function generateAnswer(question, context) {
  try {
    // Simple context-based response (no API needed)
    const contextLower = context.toLowerCase();
    const questionLower = question.toLowerCase();
    
    if (contextLower.includes('goals') && contextLower.includes('research')) {
      return "Based on the context, the main goals of AI research include reasoning, problem-solving, knowledge representation, planning, learning, natural language processing, perception, and the ability to move and manipulate objects.";
    }
    
    if (contextLower.includes('reasoning') || contextLower.includes('problem')) {
      return "The context mentions that AI research aims to develop systems capable of reasoning, problem-solving, and knowledge representation.";
    }
    
    if (contextLower.includes('learning') || contextLower.includes('machine learning')) {
      return "AI research focuses on machine learning and developing systems that can learn from data and improve over time.";
    }
    
    return "I found relevant information about AI research goals in the context. The research focuses on creating intelligent systems capable of various cognitive tasks like reasoning, learning, and problem-solving.";
    
  } catch (error) {
    console.error('❌ Failed to generate answer:', error.message);
    return "I found some information but couldn't generate a detailed answer. Here's what I found in the context...";
  }
}
...
async function cleanupOldCollections() {
  try {
    ...
    console.error('❌ Failed to clean up old collections:', error.message);
  }
}
