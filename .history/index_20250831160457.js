...
const myEmbeddingFunction = async (texts) => {
  ...
  return vectors; // array of embeddings
};

// -----------------------------
// Create collection using your own embedding function
// -----------------------------
const collection = await chroma.createCollection({
  name: 'webpage_embeddings',
  embeddingFunction: myEmbeddingFunction, // IMPORTANT: use your embedding
});
...
// -----------------------------
async function storeEmbeddings(textChunks, label) {
  for (const [i, chunk] of textChunks.entries()) {
    ...
    console.log(`Stored ${label} chunk ${i + 1}`);
  }
}
...
(async () => {
  ...
  if (headings.length > 0) {
    ...
    await storeEmbeddings(headingChunks, 'HEADINGS');
  }

  if (bodyText) {
    const bodyChunks = splitText(bodyText, 2000);
    await storeEmbeddings(bodyChunks, 'BODY');
  }
