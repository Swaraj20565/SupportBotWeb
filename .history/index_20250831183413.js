...
//     const url = 'https://www.piyushgarg.dev/about';
//     console.log(`\n🌐 Scraping: ${url}`);
...
const localData = [
  {
    ...
    chunks: [
      ...
      "I have always been passionate about technology and education.",
      "My journey has included roles as content creator, developer, entrepreneur, and innovator.",
      "I love sharing knowledge and making complex concepts easier to understand.",
      "Currently, I work on web development projects and mentor aspiring developers."
    ]
  },
  {
    label: "TECH_SKILLS",
    chunks: [
      ...
      "Experience with Python, Django, and C++ programming.",
      "Familiar with PostgreSQL, database design, and high-performance applications.",
      "Knowledge of AI/ML concepts and integrating APIs for practical solutions.",
      "Mentoring and creating educational content for developers."
    ]
  }
];
...
function generateCollectionName() {
  const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  return `local_data_embeddings_${timestamp}`;
}

// -----------------------------
// Store embeddings in ChromaDB
// -----------------------------
async function storeEmbeddingsChroma(collectionName, label, textChunks) {
  try {
    const embeddings = await generateEmbeddings(textChunks);
    ...
    const ids = textChunks.map((_, i) => `${label}_${i + 1}_${Date.now()}`);
    const metadatas = textChunks.map((chunk, i) => ({
      label,
      chunk_index: i + 1,
      total_chunks: textChunks.length,
      chars: chunk.length,
    ...
    console.log(`✅ Stored ${textChunks.length} ${label} chunks in ChromaDB`);
  } catch (error) {
    console.error(`❌ Failed to store ${label} chunks:`, error.message);
  }
}
...
// -----------------------------
// Enhanced chat using local data
// -----------------------------
async function enhancedChat(question = '') {
  try {
    console.log(`\n💬 Question: ${question}`);
    ...
    collections.sort((a, b) => b.name.localeCompare(a.name));
    const mostRecentCollection = collections[0];
    const collection = await chromaClient.getCollection({ name: mostRecentCollection.name });
...
function createChatInterface() {
  ...
  console.log('\n🤖 AI Assistant Ready!');
  console.log('💡 Ask me anything about Piyush Garg');
  console.log('🚪 Type "exit" or "quit" to end the conversation\n');
...
async function main() {
  try {
    console.log('🚀 Starting Local Data RAG System\n');

    // Generate collection name
    const collectionName = generateCollectionName();
    console.log(`📁 Collection: ${collectionName}`);

    // Store all local data into ChromaDB
    for (const dataset of localData) {
      await storeEmbeddingsChroma(collectionName, dataset.label, dataset.chunks);
    }
