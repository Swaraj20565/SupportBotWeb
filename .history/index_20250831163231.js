...
// -----------------------------
// Test ChromaDB connection with correct endpoint
// -----------------------------
async function testChromaConnection() {
  try {
    console.log('🔍 Testing ChromaDB connection...');
    // Try the heartbeat endpoint which is more reliable
    const response = await axios.get('http://localhost:8000/api/v1/heartbeat', { timeout: 5000 });
    console.log('✅ ChromaDB is running and accessible');
    console.log('📊 ChromaDB heartbeat:', response.data);
    return true;
  } catch (error) {
    console.log('ℹ️ Trying alternative endpoint...');
    try {
      // Try root endpoint
      const response = await axios.get('http://localhost:8000/', { timeout: 5000 });
      console.log('✅ ChromaDB is running (root endpoint)');
      return true;
    } catch (secondError) {
...
async function storeEmbeddingsDirect(collectionName, textChunks, label) {
  ...
  try {
    ...
    // Try different endpoints for adding data
    let response;
    try {
      ...
      console.log('ℹ️ Trying alternative add endpoint...');
      response = await axios.post(
        `http://localhost:8000/api/v1/collections/${collectionName}/upsert`,
        data,
        {
          headers: {
            'Content-Type': 'application/json'
...
async function listCollections() {
  try {
    ...
    try {
      const response = await axios.get('http://localhost:8000/api/v1/collections');
      console.log('📋 Available collections (alternative endpoint):');
      ...
      console.log('ℹ️ Could not list collections from any endpoint');
      return [];
    }
  }
}
...
async function main() {
  try {
    ...
    if (!isConnected) {
      ...
      console.log('3. Test connection: curl http://localhost:8000/api/v1/heartbeat');
      process.exit(1);
    }
    ...
    // Process and store data (just a small test)
    if (headings.length > 0) {
      const headingChunks = splitText(headings.join(' '), 500);
      // Just store first chunk for testing
      await storeEmbeddingsDirect(collectionName, [headingChunks[0]], 'HEADINGS_TEST');
    }
